...
Eigen::MatrixXi fill(int L, double P, double seed);/*Función que llena la matriz con las casillas abiertas (1) y cerradas (0). 

void hood(const  Eigen::Ref<Eigen::MatrixXi> I,  Eigen::Ref<Eigen::MatrixXi> F, int L, int ii, int jj, int target);/*Esta función a partir de una casilla (i, j) de la matriz I, crea una matriz F con un clouster formado por casillas con el valor de "target" y todas las demás casillas fuera del clouster particular son ceros. */

Eigen::MatrixXi Path(const Eigen::Ref< Eigen::MatrixXi> I, int L, int scanning, int target);/*Este método toma la matriz de entrada I y devuelve otra matriz con todos los clousters del valor target (percolantes o no) que inician en la arista izquierda si el valor de scanning es 1, o desde la derecha si el valor de scanning es 0. */

Eigen::MatrixXi clouster_matrix(const Eigen::Ref< Eigen::MatrixXi> I, int L);/*Este método toma la matriz I, y nos devuelve lo que llamé"la matriz de percolación, que estará llena con el número 2 en todas las casillas que hagan parte de un clouster percolante, los demás valores son ceros." */

bool Percolation(const Eigen::Ref<Eigen::MatrixXi> I, int L); /*Este método recibe la matriz de percolación y nos dice si existe un 
...
void hood( const Eigen::Ref<Eigen::MatrixXi> I,  Eigen::Ref<Eigen::MatrixXi> F, int L, int ii, int jj, int target){

  if (ii>=L || ii<0 || jj>=L || jj<0 || I(ii,jj)!=target || F(ii,jj)==target) {return;} /*Si el índice está fuera de los límites, si
...
////////////////////////////////////////////////////////////////////////////////////
Eigen::MatrixXi Path(const Eigen::Ref< Eigen::MatrixXi> I, int L, int scanning, int target){


  Eigen::MatrixXi F(L,L);                            /*Se crea una matriz de ceros F, la cual se llenará con todos los clousters
...
Eigen::MatrixXi clouster_matrix(const Eigen::Ref< Eigen::MatrixXi> I, int L){
  ...
  H= I.transpose();    //Declaramos H, que será la transpuesta de I, esto para identificar los clousters verticales usando el mismo alg                         oritmo,  

  Eigen::MatrixXi J(L,L);
  J = Path(H, L, 1, 1);//Se crean los caminos de números desde la izquerda de la transpuesta de I, lo cual es equivalente a crearlos de                         sde la parte de arriba de I.

  Eigen::MatrixXi med(L, L);//Para crear la matriz percolante sumamos F y la transpuesta de F, las casillas que hagan parte de un clous                             ter, al sumarlas, el resultado debe ser 2, sin embargo esto no es suficiente, porque aún pueden haber clou                             sters que no son percolantes. 
  med= F + J.transpose();

  Eigen::MatrixXi F1(L,L); //Para solucionar esto, creamos una matriz F1, que tomará la matriz med y le creará los caminos buscando el                              número 2 pero desde la derecha, grantizando que esos caminos solo pueden corresponder a clousters percolan                            tes, por ende F1 será la matriz de percolación, nótese que si el sistema no percola, la matriz F1 será nula                                
  F1 = Path(med, L, 0, 2);
